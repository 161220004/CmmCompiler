...
int ensure(Operand* op, InterCode* current) {
  ...
  if (index > -1) { // op is already in register[index]
    return index;
    ...
    if (op->kind == OP_CONST) { // 常数
      ...
      if (memIndex == -1) { // 不在变量内存表内，则添加到变量内存表
...
int spillFarthestReg(InterCode* current) {
  ...
  int useNo = 0; // 使用顺序
  for (int i = R_T0; i < R_S0; i++) {
    while (!isBlockHead(current->next)) { // 还在基本块内（没进入下一个基本块）
      if (current->kind == IR_WRITE || current->kind == IR_RETURN) {
        if (strcmp(current->one.op->name, regs[i].var) == 0) {
          useNo += 1;
          regs[i].useBit = useNo;
        }
      } else if (current->kind == IR_ASSIGN) {
        if (strcmp(current->two.op2->name, regs[i].var) == 0) {
          useNo += 1;
          regs[i].useBit = useNo;
        }
      } else if (current->kind == IR_ADD || current->kind == IR_SUB || current->kind == IR_MUL || current->kind == IR_DIV) {
        if (strcmp(current->three.op2->name, regs[i].var) == 0 || strcmp(current->three.op3->name, regs[i].var) == 0) {
          useNo += 1;
          regs[i].useBit = useNo;
        }
      }
...
bool isBlockHead(InterCode* code) {
  ...
  if (code->kind == IR_GOTO || code->kind == IR_IF) return true;
  return false;
}
